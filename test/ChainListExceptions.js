...
contract('ChainList', function(accounts) {
  ...
  // Test case: buying an article when there are no articles
  it("should throw an exception if you are buying an article when there are no articles for sale", function() {
    ...
    .catch(function(error) {
      assert(error.message.indexOf('revert') >= 0, "error should be reverted");
    }).then(function(){
      return chainListInstance.getNumberOfArticles();
    }).then(function(data){
      assert.equal(data.toNumber(),0,"number of articles must be a zero");
    });
  });

  // Test case: buying a non-existing article
  it("should throw an exception if you are buying an article when there is no article with such ID", function() {
    return ChainList.deployed().then(function(instance) {
      chainListInstance = instance;
    ...
    .catch(function(error) {
      ...
      assert.equal(data[1], seller,"article seller must be " + seller);
