App = {
  ...
  reloadArticles: function() {
    ...
    App.contracts.ChainList.deployed().then(function(instance) {
      ...
      for (var i=0; i < articleIds.length; i++) {
        var currentId = articleIds[i];
        chainListInstance.articles(currentId).then(function(article) {
          App.displayArticle(
            article[0],
            article[1],
            article[3],
  ...
  displayArticle: function(id, seller, name, description, price) {
      ...
      articleTemplate.find('.btn-buy').text(price + " ETH");
  ...
  buyArticle: function() {
    ...
    let articleId = $(event.target).data('id');
    let price = parseFloat($(event.target).data('value'));
